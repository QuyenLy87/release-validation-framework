...
public class ManifestPatternTester {
	...
	public ManifestPatternTester(final ValidationLog validationLog, final ResourceProvider resourceManager,
			final ManifestFile manifestFile, BuildType buildType, final TestReportable report) {
	...
	public void runTests() {
		...
		if (manifestFile == null || manifestFile.getListing() == null) {
			...
			report.addError("0", startTime, MANIFEST, MANIFEST, MANIFEST, MANIFEST_STRUCTURE_TEST, "",
					"No Manifest File Found", MANIFEST, null);
		} else {
			...
			validationLog.info(
					"Manifest file structure testing for {} files and {} folders completed in {} milliseconds.",
					fileCounter, folderCounter, (new Date().getTime() - startTime.getTime()));
		}
	}

	private void testStructure(final List<Folder> folders, final AtomicInteger folderCounter,
			final AtomicInteger fileCounter, final Date startTime) {
		System.out.println("===========================: " + resourceManager.getFilePath());
		if (folders.isEmpty())
			return;
		for (final Folder folder : folders) {
			...
			if (!match) {
				...
				report.addError(folderCounter + "-" + fileCounter, startTime, name, name, name, MANIFEST_STRUCTURE_TEST,
						"", "No Folder Found", name, null);
			} else {
			...
			for (final FileElement file : files) {
				...
				if (!(resourceManager.match(filename))) {
					validationLog.assertionError("Invalid package structure expected file at {} but found none",
							filename);
					report.addError(folderCounter + "-" + fileCounter, startTime, filename, filename, filename,
							MANIFEST_STRUCTURE_TEST, "", "No File Found", filename, null);
				} else {
					report.addSuccess(folderCounter + "-" + fileCounter, startTime, filename, filename, "",
							MANIFEST_STRUCTURE_TEST, "");
				}
			}
	...
	private void checkFileName(String fileName, String executionId, final Date startTime) {
		if(BuildType.ALPHA == this.buildType || BuildType.BETA == this.buildType){
			
		}
		report.addSuccess(executionId, startTime, fileName, "", "", MANIFEST_STRUCTURE_TEST, "");
	}
}
