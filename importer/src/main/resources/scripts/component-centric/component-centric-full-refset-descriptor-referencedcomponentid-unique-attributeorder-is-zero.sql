...
********************************************************************************/
	...
	select
		...
		'<ASSERTIONUUID>',
		a.moduleid,
		concat('Refset: id=',a.id,' ReferencedComponentId = ',a.referencedcomponentid,' has no record where attributeOrder = 0 in REFSET DESCRIPTOR FULL')
	from
	    curr_refsetDescriptor_f a, (
            select  referencedcomponentid ,attributeorder,count(*) total
            from curr_refsetDescriptor_f
        ...
        and not exists (
            select 1
            from curr_refsetDescriptor_f c
